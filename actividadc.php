...
<div class="back_btn"><a href="index.php" data-transition="flip"><img src="images/icons/black/back.png" alt="" title="" /></a></div> 
    ...
    <div role="main" class="ui-content">
	   ...
	   <div class="form_logo" style="font-size:28px;">ACTIVIDADES DE LOS COMERCIALES</div>
      ...
      <ul class="features_list_detailed">
		...
		//$consulta = "SELECT DISTINCT usuarios.id_usuario, usuarios.nombre_comp, COUNT(visitas.id_visitas) AS TOTAL, SUM(visitas.visita_efec) AS EFECTIVAS, MIN(visitas.km) AS KM_INICIAL, MAX(visitas.km) AS KM_ACTUAL FROM (usuarios INNER JOIN visitas ON usuarios.id_usuario=visitas.id_usuario AND CAST(visitas.fecha_hora_vis AS DATE) = '$fecha_hora') WHERE visitas.cliente NOT LIKE 'Por Favor Espere' GROUP BY usuarios.id_usuario ";
		$consulta = "SELECT DISTINCT usuarios.id_usuario, usuarios.nombre_comp, group_concat(`cliente`) AS CLIENTES, COUNT(visitas.id_visitas) AS TOTAL, SUM(visitas.visita_efec) AS EFECTIVAS, MIN(visitas.km) AS KM_INICIAL, MAX(visitas.km) AS KM_ACTUAL FROM (usuarios INNER JOIN visitas ON usuarios.id_usuario=visitas.id_usuario AND CAST(visitas.fecha_hora_vis AS DATE) = '$fecha_hora') WHERE visitas.cliente NOT LIKE 'Por Favor Espere' GROUP BY usuarios.id_usuario ";
					...
					while($row=mysqli_fetch_array($resultado)){ 
				       ...
				       ?>
						 <li>
							  ...
							  <p style="color:#FFFFFF; font-size:15px;"><b>Clientes: <?php echo strtoupper($row['CLIENTES']); ?></b></p>
